...
sub record_tweet {
  ...
  SUBSTITUTE:{
    ...
    record_tweet($retweeted_r, $weak) if defined($retweeted_r);
    record_tweet($quoted_r, $weak) if defined($quoted_r);
}
...
sub record_media {
    ...
    if ( defined($r->{"additional_media_info"}->{"source_user"}->{"user_results"}->{"result"})
	 ...
	 && ($r->{"additional_media_info"}->{"source_user"}->{"user_results"}->{"result"}->{"__typename"} eq "User")
	 && defined($r->{"additional_media_info"}->{"source_user"}->{"user_results"}->{"result"}->{"rest_id"}) ) {
	my $parent_author_r = $r->{"additional_media_info"}->{"source_user"}->{"user_results"}->{"result"};
	record_user($parent_author_r, $weak);
    }
}
